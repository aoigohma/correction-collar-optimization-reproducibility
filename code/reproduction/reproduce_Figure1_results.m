...
for iExp = 1:numel(F1)
    ...
    Fit = ccrepro.fit_peakf_series( ...
        ...
        sprintf('%s Figure1', experimentID));

    fitCoeff = Fit.fit_coeff;
    fitCurve = Fit.fit_curve;
    ...
    thetaFitValidDeg = Fit.theta_fit_deg;
    thetaFitValidDeg(~Fit.valid_fit) = NaN;
    ...
    W95Deg = Fit.W95_deg;
    angleFineDeg = Fit.angle_fine_deg(:)';
...
v = ccrepro.get_param(S, fieldName, defaultValue);
end

function step = local_constant_step(angleDeg, experimentID)
step = ccrepro.constant_step(angleDeg, experimentID);
end
