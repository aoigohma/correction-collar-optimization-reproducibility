...
for iExp = 1:numel(F234)
    ...
    FitRef = local_fit_peakf_series(angleDeg, peakfRef, xRaw, xFine, angleFineDeg, fitDx, ...
    ...
    for d = 1:numel(depthList)
        ...
        FitTarget = local_fit_peakf_series(angleDeg, peakfTarget, xRaw, xFine, angleFineDeg, fitDx, ...
...
Fit = ccrepro.fit_peakf_series( ...
    angleDeg, peakf, polyOrder, fitDx, r2Threshold, NaN, ...
...
[relativeScore, numeratorScore, refInsideScan, nNegative, nOverOne] = ...
    ccrepro.relative_fitted_score(thetaRef, angleFineDeg, fitCurve, fittedMaxScore, validFit);
end
...
longest = ccrepro.longest_run(below, valid, segmentID);
end
...
q = ccrepro.prctile_finite(vals, p);
end
