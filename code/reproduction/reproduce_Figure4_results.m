...
%   6) HISTORICAL ONLY (not in current manuscript): optional linear
%      mixed-effects models with mouse random intercept:
...
%   Figure4_LME_coefficients.csv      [historical opt-in only]
%   Figure4_LME_model_comparison.csv [historical opt-in only]
...
p.addParameter('RunLME', false, @(x) islogical(x) && isscalar(x)); % HISTORICAL ONLY; excluded from current manuscript
...
for iExp = 1:numel(F234)
    ...
    for c = 1:numel(conditionNames)
        ...
        for d = 1:numel(depthList)
            ...
            Fit = local_fit_peakf_series(angleDeg, peakfThis, xRaw, xFine, angleFineDeg, fitDx, ...
...
%% HISTORICAL ONLY: mixed-effects models are excluded from the current manuscript.
% Opt in explicitly with 'RunLME',true and a SEPARATE output directory.
if opt.RunLME
...
if ~opt.RunLME && (isfile(fullfile(outputDir, 'Figure4_LME_coefficients.csv')) || ...
        isfile(fullfile(outputDir, 'Figure4_LME_model_comparison.csv')))
    warning(['Pre-existing historical LME CSVs remain in this output directory. ', ...
        'They were NOT generated by the current run and are NOT current-manuscript outputs. ', ...
        'For an unambiguous clean run, use a new output directory.']);
end
...
if opt.WriteMAT
    ...
    reproduction_parameters.RunHistoricalLME = opt.RunLME;
    if opt.RunLME
...
if opt.RunLME
    fprintf('\nHISTORICAL ONLY: optional LME analysis was requested; it is not part of the current manuscript.\n');
else
    fprintf('\nCurrent-manuscript scope: historical LME not run.\n');
end
...
function Fit = local_fit_peakf_series(angleDeg, peakf, xRaw, xFine, angleFineDeg, fitDx, ...
    polyOrder, r2Threshold, W95Fraction, experimentID, conditionName, depthUm)
% Shared implementation; xRaw/xFine/angleFineDeg are retained in the signature
% for compatibility with the originally verified standalone script. The exact
% configured fitDx is passed through unchanged so the fine-grid coordinates
% exactly match the verified standalone analysis.
Fit = ccrepro.fit_peakf_series( ...
    angleDeg, peakf, polyOrder, fitDx, r2Threshold, W95Fraction, ...
    sprintf('%s %s %dum', experimentID, conditionName, depthUm));
end
...
[M, experimentsUsed] = ccrepro.complete_matrix(T, valueVar, factorVar, factorLevels);
end
...
pVal = ccrepro.friedman_p(X, 'MinRows', 3, 'ReturnNaNWhenTooSmall', true);
end
